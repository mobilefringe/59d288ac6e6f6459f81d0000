...
function submit_contest(data, slug) {
    ...
    console.log(data);
    $.ajax({
        url: host + "/contests/" + slug + "/json_entry",
        type: "POST",
        data: data,
...
function getStoreAlphabets(collection){
      var initials_list = [];
      onlyLetters = [];
      var final_initials_list = [];
      
      initials_list = _.map(collection, function(o){ return o.name.charAt(0).toUpperCase();});
      
    _.forEach(initials_list, function(value, key) {
        if (!parseInt(value)) {
           onlyLetters.push(value);
        }
    });
    

    final_initials_list = _.uniq(onlyLetters); 
      
    return final_initials_list;
